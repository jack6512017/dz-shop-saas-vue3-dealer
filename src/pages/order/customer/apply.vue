	...
	<view>
		...
		<view class="form-content">
			<dz-form ref="formRef" :model="form" label-width="160">
				<dz-form-item label="退款原因" prop="refund_reason">
					<dz-input v-model="form.refund_reason" clearable border="none" placeholder="请输入退款原因"></dz-input>
				</dz-form-item>
				<dz-form-item :label="infoData.warehouse_is_pickup == 1 ? '退增库存' : '退款金额'" :required="false">
				...
				<dz-form-item label="问题描述(选填)" :required="false">
					<dz-textarea
...
function submit() {
	unref(formRef)
		...
		.then(() => {
			...
			data.isLoading = true;
			form.id = data.infoData.id;
			form.refund_type = 1;
			uni.$api.http
				.post(uni.$api.apiDealer.orderCustomerApply, {
					...form,
				})
				.then(() => {
					uni.$api.toast('提交成功');
