	...
	<view>
		...
		<view class="form-content">
			<dz-form ref="formRef" :model="form" label-width="165">
				...
				<dz-form-item label="支行信息" prop="branch_bank_name" :borderBottom="true">
...
async function getDetail() {
	await uni.$api.http
		.get(uni.$api.apiDealer.bankAccountView, {
			params: {
				id: data.id,
			},
		})
...
async function handleUpdate(params) {
	await uni.$api.http
		.put(uni.$api.apiDealer.bankAccountUpdate, params, {
			params: {
				id: params.id,
			},
		})
...
async function handleCreate(params) {
	await uni.$api.http
		.post(uni.$api.apiDealer.bankAccountCreate, params)
		.then((res) => {
			uni.$api.toast('添加成功');
			if (source.value) {
				uni.$emit('getBankAccount', res.data);
