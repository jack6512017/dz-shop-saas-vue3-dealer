...
function getWithdrawConfig() {
	uni.$api.http.get(uni.$api.apiDealer.brokerageWithdrawConfig).then((res) => {
...
async function getList() {
	...
	await uni.$api.http
		.get(uni.$api.apiPay.yunzhanghuDealerCardIndex, {
			params: {
				page: page.value,
				page_size: 15,
			},
...
async function actionClick(index: number) {
	await uni.$api.http
		.post(
			uni.$api.apiPay.yunzhanghuDealerCardDelete,
			{},
			{
				params: {
...
function handleUpdate(type: string, id?: string) {
	uni.$api.router.push({
		route: uni.$api.routesConfig.yunzhanghuMemberCardUpdate,
		query: {
			type: type,
			id: id,
			source: source.value,
