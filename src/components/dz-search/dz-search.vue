<template>
	...
	>
		...
		<view :style="[actionStyle]" class="dz-action" :class="[showActionBtn || show ? 'dz-action-active' : '']" @tap.stop.prevent="custom">{{
			actionText
		}}</view>
	</view>
</template>
...
const emit = defineEmits(['update:modelValue', 'change', 'clear', 'search', 'custom', 'focus', 'blur', 'click']);
const keyword = ref<string>('');
const show = ref<boolean>(false);

// 标记input当前状态是否处于聚焦中，如果是，才会显示右侧的清除控件
const focused = ref<boolean>(props.focus);
// 绑定输入框的值
